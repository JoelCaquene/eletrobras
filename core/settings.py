...
# A URL do seu app Heroku não é necessária para o teste local
...
INSTALLED_APPS = [
    ...
    # Apps de terceiros para deploy (desativado para teste local)
    # 'cloudinary_storage',
    # 'cloudinary',
]
...
MIDDLEWARE = [
    ...
    # WhiteNoise deve vir logo após SecurityMiddleware (desativado para teste local)
...
# --------------------------------------------------------------
# Início da seção para configuração do banco de dados local
DATABASES = {
    'default': {
...
# A configuração do Heroku foi removida para teste local
...
# Configuração de armazenamento de arquivos (desativado para teste local)
...
# Configurações do Cloudinary (não são necessárias para teste local)
...
# Para segurança CSRF em produção (não necessária para teste local)
